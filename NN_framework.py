...
class NeuralNet:
    ...
    def __init__(self, X, Y, nodes, activations, task, X_test=None, Y_test=None):
        '''
        Parameters - 
        X (n, m) (matrix) --> n: number of features | m: number of examples
        Y (1, m) (matrix) --> m: number of examples
        nodes (list) --> list containing no. of nodes in each layer
        activations (list) --> list containing activation type in each layer
        task (string) --> task to be performed (binary/regression/multilabel/multiclass)
        X_test (matrix) (default None) --> Test set
        Y_test (matrix) (default None) --> Test set target values
        '''
        ...
        for act in activations:
            assert act in ['relu', 'lrelu', 'sigmoid', 'tanh', 'softmax', 'linear'], "Invalid activation given"
        self.X = X
        self.Y = Y
        self.nodes = nodes
